...
@section('content')
<form action="{{ route('barang.update', $barang->idbarang) }}" method="POST" class="space-y-4">
  @csrf @method('PUT')
  ...
  <div>
    ...
    <select name="idsatuan"
            class="w-full border-gray-300 rounded-lg shadow-sm mt-1 focus:ring focus:ring-blue-200" required>
      @foreach ($satuan as $s)
